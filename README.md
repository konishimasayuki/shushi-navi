...
`/api/data` 経由でUpstash Redisに保存されます。

- **管理者(`b`/`b`)・打ち子ログイン**: 実データ。Redisに保存され、誰がアクセスしても
  同じデータが共有・保持される。初回は空の状態からスタート
- **オーナーデモ(`z`/`z`)ログイン**: `lib/demoData.js` の見本データ（約1年分・84件の
  稼働記録入り）のみを表示。Redisには一切アクセスせず、ログインし直すたびに
  同じ見本データにリセットされる

## ロール

- 管理者(`b`/`b`): 打ち子登録・店舗/機種/タグのマスタ管理が可能。実データを使用
- オーナーデモ(`z`/`z`): 見本データのみを閲覧できるデモ専用アカウント
- 打ち子: 設定画面で登録した名前・ID・パスワードでログイン。自分の稼働記録のみ入力可能。実データを使用
